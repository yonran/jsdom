...
/*
 * Note: in Webkit, length is an own enumerable property and item is a
 * enumerable, non-own property. In contrast, in Gecko, both length
 * and item are non-own enumerable properties. (contrast this to array.length,
 * which is an own, non-enumerable property).
 */
...
core.NodeList.prototype = {
  ...
  indexOf: function(node) {
    this.update();
    var len = this._snapshot.length;

    for (var i = 0; i < len; i++) {
      if (this._snapshot[i] == node) {
        return i;
      }
    }
...
/** Returns a proxy for obj. When proxy[i] is accessed, returns obj.item(i).
 */
core.createArrayLike = function proxyArrayLike(obj) {
  var proto = Object.getPrototypeOf(obj);
  var boundFns = {};
  for (var prop in obj) {
    if ('function' === typeof obj[prop]) {
      boundFns[prop] = (function() {
        var val = obj[prop];
  ...
  var handler = {
    ...
    has:          function(name) { return hasIndex(name) || name in obj; },
    hasOwn:       hasOwn,
    get:          get,
    set:          set,
    getPropertyNames: getPropertyNames,
    enumerate:    getPropertyNames,  // all properties are assumed enumerable
    keys: getOwnPropertyNames 
  };
  return proxy.create(handler, proto);

  function hasIndex(name) {
    var asInt = parseInt(name);
    if (name == asInt)  // coercing equality
      return asInt < obj.length;
  }
  function hasOwn(name) {
    return hasIndex(name) || ({}).hasOwnProperty.call(obj, name);
  }
  function get(receiver, name) {
    var fn = boundFns[name];
    if (null != fn)
      return fn;
    var val = obj[name];
    if (undefined != val)
  ...
  function set(receiver, name, val) {
    var asInt = parseInt(name);
    if (name == asInt)  // coercing equality
      return;  // set item is no-op.
    obj[name] = val;
  }
  function getOwnPropertyNames() {
    ...
    var orig = Object.getOwnPropertyNames(obj);
    return a.concat(orig);
  }
  function getPropertyNames() {
    var a = getOwnPropertyNames();
...
core.Node = function Node(ownerDocument) {
  ...
  this._childrenList = core.createArrayLike(new core.NodeList(this, function() {
    return self._childNodes.filter(function(node) {
      return node.tagName;
    });
  }));

  this._childNodesList = core.createArrayLike(new core.NodeList(this, function() {
...
core.Element.prototype = {
  ...
  getElementsByTagName: function(/* string */ name) {
    ...
    return core.createArrayLike(new core.NodeList(this._ownerDocument || this, core.mapper(this, filterByTagName, true)));
  },
};
...
core.Document.prototype = {
  ...
  getElementsByTagName: function(/* string */ name) {
    ...
    return core.createArrayLike(new core.NodeList(this.documentElement || this, core.mapper(this, filterByTagName, true)));
  }
};
